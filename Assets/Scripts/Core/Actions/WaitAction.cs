...
namespace XianXia.Core.Actions
{
    ...
    {
        public WaitAction(ActionId id, EntityId subject, OrderId sourceOrderId, ulong durationTicks)
        {
            Id = id;
            Subject = subject;
        ...
        public Result CanStart(SimulationWorld world)
        {
            ...
            if (life.IsDead || life.IsRemoved || life.IsIncapacitated)
                return Result.Failure(ErrorCode.ActionCannotStart, "Subject cannot act.", life.State.ToString());

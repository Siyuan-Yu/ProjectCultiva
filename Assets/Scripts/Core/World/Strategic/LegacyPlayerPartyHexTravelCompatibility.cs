...

    {
...

        {
...

            {
                ...
                // 不再用 hex 直线距离）；进入后再聚合 LegacyPresenceHex。
...

            {
                ...
                if (fromSite.OccupiesLegacyHex(goalHex))
                ...
                motion.BeginLegacySiteDepartureTravel(
            ...
            var normalRouteOk =
                ...
                HexPathfinder.TryFindPath(
                    ...
                    world.LegacyHexWorld, startHex, goalHex, PathScratch, mode, blockedSiteHexes) &&
            ...
            world.PlayerPartyTravel.BeginLegacyHexAutoTravel(
...

        {
            ...
            if (motion == null || party == null || !motion.IsMoving ||
                ...
                !motion.IsLegacySiteDeparturePending || string.IsNullOrEmpty(motion.SiteId) ||
            ...
            if (!BackgroundCharacterSiteDepartureResolver.TryResolveDepartureHex(
                    ...
                    world, site, motion.LegacyDestinationHex, locallyReachableExitHexes, out var exitHex) ||
            ...
            if (!HexPathfinder.TryFindPath(world.LegacyHexWorld, motion.LegacyCurrentHex, footprintHex, PathScratch) ||
            ...
            if (!exitHex.Equals(motion.LegacyDestinationHex) &&

                (!HexPathfinder.TryFindPath(world.LegacyHexWorld, exitHex, motion.LegacyDestinationHex, tail) || tail.Count < 1))
            ...
            motion.ReplaceLegacySiteDeparturePlan(PathScratch, footprintHex, exitHex, boundary);
...

        {
            ...
            if (!startHex.Equals(departureFootprintHex) &&
                ...
                HexPathfinder.TryFindPath(world.LegacyHexWorld, startHex, departureFootprintHex, scratch, HexTravelMode.Ground, blocked) &&
            ...
            if (!HexPathfinder.TryFindPath(
                    ...
                    world.LegacyHexWorld, exitHex, goalHex, scratch, HexTravelMode.Ground, blocked) ||
...

        {
            ...
            // GroundBaseStepTicks remains the legacy protocol meaning (8 ticks per adjacent-center

            // reference distance). The current pure formula preserves the same operation order.

            return PlayerPartyTravelRuntimeService.WorldUnitsPerTick(hexSize);
...

        {
            ...
            // egress commit（AtWorldPosition + route 对齐 LegacyDestinationHex），后续段继续由 World
            ...
            var pos = motion.IsLegacySiteDeparturePending &&

                      motion.LocationKind == PlayerPartyLocationKind.AtWorldSite

                ? motion.LegacySiteDepartureVirtualPosition

                : motion.LegacyUsesTravelPresentation &&

                  motion.LocationKind == PlayerPartyLocationKind.AtWorldSite

                    ? motion.LegacyTravelPresentationPosition

                    : motion.WorldPosition;

            var previousDerived = motion.LegacyCurrentHex;

            var isSiteDepartureVirtual = motion.IsLegacySiteDeparturePending &&
...

            {
                ...
                if (!motion.TryGetActiveLegacyHexSegmentWorld(hexSize, out var fromPos, out var toPos))

                {
...

                    {
                        ...
                        previousDerived = motion.LegacyCurrentHex;
...

                {
                    ...
                    motion.IncrementLegacyHexPathIndex();

                    if (motion.LegacyHexSegmentIndex >= motion.LegacyHexPathCount - 1)
...

                {
...

                    {
                        ...
                        var previousHex = motion.LegacyCurrentHex;

                        motion.SetLegacySiteDepartureVirtualPosition(pos, hexSize);

                        if (ShouldCommitSiteDepartureBoundaryCrossing(world, motion, previousHex, motion.LegacyCurrentHex))
                        ...
                        isSiteDepartureVirtual = motion.IsLegacySiteDeparturePending &&
...

                {
...

                    {
                        ...
                        motion.SetLegacyHexSegment(motion.LegacyHexSegmentIndex, virtualProgress);
                ...
                // BoundaryContact，恰在共享边中点）时 WorldToHex 会 tie 到错误格，把 LegacyCurrentHex

                // 从已提交的 LegacyDestinationHex 拉走 → 后续段推进错乱。跨段才更新（段完成分支用

                // toPos=下一 hex center，WorldToHex 无 tie）。

                var midDerived = motion.LegacyCurrentHex;
                ...
                motion.SetLegacyHexSegment(motion.LegacyHexSegmentIndex, progress);
...

        {
            ...
            if (motion.IsLegacySiteDeparturePending &&
                ...
                return motion.LegacySiteDepartureVirtualPosition;

            if (motion.LegacyUsesTravelPresentation &&

                motion.LocationKind == PlayerPartyLocationKind.AtWorldSite)

                return motion.LegacyTravelPresentationPosition;
        ...
        /// （LegacySiteDepartureBoundaryEntry）消耗 distance budget 推进；到达后

        /// <see cref="CommitSiteDepartureBoundaryCrossing"/> 正式 egress（AtWorldPosition +

        /// LegacyCurrentHex=LegacySiteDepartureExitHex + route 对齐），随后用剩余预算继续推进 AtWorldPosition 段。

        /// 不使用 LegacySiteDepartureVirtualPosition（旧 presentation 状态，不重新变成 physical truth）；
...

        {
            ...
            motion.TryUpdateLegacyWorldPositionWithinSite(motion.SiteId, new WorldVec2(nx, ny));
        ...
        /// （FormalConnection.LegacyDestinationHex）。在 LegacyHexPath 中定位该格并把 LegacyHexSegmentIndex 设为其起点段；

        /// 找不到（防御）才退回旧 +1 推进。exactly once、不重复推进、不跳过下一段、不依赖

        /// WorldToHex(BoundaryContactWorld) tie-break。

        /// 背景：LocalVisible departure 全程 LegacyHexSegmentIndex 恒 0（SyncSegmentProgressFromWorldPosition

        /// 只写 progress），旧 SetLegacyHexSegment(LegacyHexSegmentIndex+1,0) 在 LegacyHexPath 前部含多个 footprint hex
...

        {
...

            {
                ...
                motion.SetLegacyHexSegment(idx, 0f);
...

        {
            ...
            var boundaryEntry = motion.LegacySiteDepartureBoundaryEntry;

            var exitHex = motion.LegacySiteDepartureExitHex;

            // Phase 5R-B6.5-A：Route progress truth = LegacySiteDepartureExitHex（= FormalConnection.

            // LegacyDestinationHex，已提交的 first outside hex），不依赖 WorldToHex(BoundaryContactWorld)

            // 的 perimeter tie。Canonical physical truth = boundaryEntry（WorldPosition）。

            motion.SetLegacyWorldPositionAndHex(boundaryEntry, exitHex);

            AlignRouteProgressAfterSiteEgress(motion, exitHex);

            motion.ClearLegacySiteDeparturePending();
...

        {
            ...
            if (!motion.IsLegacySiteDeparturePending || string.IsNullOrEmpty(motion.SiteId))
            ...
            return site.OccupiesLegacyHex(previousHex) && !site.OccupiesLegacyHex(newHex);
...

        {
            ...
            var destSiteId = motion.LegacyDestinationSiteId ?? string.Empty;

            if (string.IsNullOrEmpty(destSiteId))

                destSiteId = TryCanonicalizeFootprintHexDestination(

                    world, motion.LegacyDestinationHex, destSiteId, out _);



            if (!LegacyWorldSiteHexLocationCompatibility.TryDetectDestinationSiteIngress(
            ...
            motion.CommitLegacySiteArrivalAuthority(site.SiteId, presentationPos, newDerived);
...

        {
            ...
            if (LegacyWorldSiteHexLocationCompatibility.TryGetSiteAtHex(world, derived, out var site) &&
...

            {
...

                {
                    ...
                    motion.SetLegacyWorldPositionAndHex(pos, derived);
...

                {
                    ...
                    motion.CommitLegacySiteArrivalAuthority(site.SiteId, pos, derived);
                ...
                // 非目标 Site：Context 进入，但保留同一个 Travel Order，并立即由 LegacyHexPath
...

            {
                ...
                if (world.Strategic.Sites.TryGet(motion.SiteId, out var currentSite) &&
                    ...
                    currentSite.OccupiesLegacyHex(derived))

                {

                    motion.SetLegacyTravelPresentation(pos, derived);
        ...
        /// Phase 5R-B7A：把 LegacyHexPath 中连续的非目标 Site footprint 段解释为一次
...

        {
            ...
            if (!site.OccupiesLegacyHex(ingressFootprintHex))

                return false;

            if (!string.IsNullOrEmpty(motion.LegacyDestinationSiteId) &&

                string.Equals(motion.LegacyDestinationSiteId, site.SiteId, StringComparison.Ordinal))
            ...
            var searchFrom = Math.Max(0, motion.LegacyHexSegmentIndex);
            ...
            while (lastFootprintIndex + 1 < path.Count &&
                   ...
                   site.OccupiesLegacyHex(path[lastFootprintIndex + 1]))
            ...
            motion.CommitLegacySiteArrivalAuthority(
            ...
            motion.PlanLegacyThroughSiteDeparture(footprintHex, exitHex, boundaryEntry);
        ...
        /// LegacyDestinationSiteId 只是 Outdoor region identity，不触发 Site ingress。
        ...
        /// 不 SnapToLegacyHexCenter、不 ClearPartyWorldPresentationCacheForOpenWorld、不重新 Materialize。
...

        {
            ...
            if (!motion.HasContinuousPhysicalDestination && !motion.LegacyCurrentHex.Equals(motion.LegacyDestinationHex))

                return Result.Failure(ErrorCode.InvalidOperation, "Not at destination hex yet.");

            var destHex = motion.LegacyDestinationHex;

            var wasContinuousSurfaceTravel = motion.HasContinuousPhysicalDestination;



            if (motion.HasContinuousPhysicalDestination &&

                WorldSitePhysicalRegionQuery.TryResolve(world, motion.WorldPosition, out var surfaceSite))

                motion.SetCurrentOutdoorWorldSiteContext(surfaceSite.SiteId);
            ...
            if (motion.HasPosition && wasContinuousSurfaceTravel)

            {
                ...
                for (var i = 0; i < motion.TravelingMembers.Count; i++)
                    ...
                    world.WorldPresence.SetAtWorldPosition(
                        ...
                        motion.TravelingMembers[i], motion.WorldPosition, motion.LegacyCurrentHex,

                        navigation?.SurfaceId ?? string.Empty);
...

        {
            ...
            var hexBefore = motion.LegacyCurrentHex;
            ...
            // Phase 5R-B3B：Site 目标<b>不</b> SnapToLegacyHexCenter —— Physical Position 保持段行进中

            // TryCommitSiteArrivalIngress → CommitLegacySiteArrivalAuthority 已提交的跨边界连续位置。
...

            {
...

                {
                    ...
                    ApplyTravelingMembersAtHex(world, motion.LegacyCurrentHex);
...

                {
...

                    {
                        ...
                        if (!motion.TrySetAtLegacyWorldSitePreservingWorldPosition(
...

                    {
                        ...
                        motion.SetAtLegacyWorldSite(site.SiteId, destHex, hexSize);
...

            {
                ...
                motion.SnapToLegacyHexCenter(destHex, hexSize);
...

        {
            ...
            var normalMotion = world.PlayerPartyTravel;

            if (ContinuousOutdoorGameplayPolicy.IsNormalContinuousOutdoor(world) &&

                normalMotion?.HasPosition == true &&

                world.SurfaceGround.TryResolveContaining(normalMotion.WorldPosition, out _) &&

                (normalMotion.LocationKind != PlayerPartyLocationKind.AtWorldSite ||

                 (world.Strategic.Sites.TryGet(normalMotion.SiteId, out var outdoorSite) &&

                  WorldSiteOutdoorMigrationPolicy.UsesContinuousOutdoorSurface(outdoorSite))))
...

            {
...

                {
                    ...
                    var hexSizePeek = world.LegacyHexWorld != null && world.LegacyHexWorld.HexSize > 0f
                    ...
                    // 无位置时才回退 legacy LegacyPresenceHex（兼容，随 5R-D 删除）。

                    if (!world.PlayerPartyTravel.HasPosition ||

                        !world.PlayerPartyTravel.TrySetAtLegacyWorldSitePreservingWorldPosition(

                            focusSite.SiteId,

                            world.PlayerPartyTravel.ResolveLegacyTravelPresentationWorld(hexSizePeek)))
        ...
        /// Forbids Cancel / CompleteMove / Snap / BeginTravel / SetAtLegacyWorldSite (clears path).
...

        {
            ...
            var hex = motion.LegacyCurrentHex;
...

        {
...

            {
                ...
                var size = world.LegacyHexWorld != null && world.LegacyHexWorld.HexSize > 0f ? world.LegacyHexWorld.HexSize : 1f;

                var derived = HexMath.WorldToHex(continuousMotion.WorldPosition.X, continuousMotion.WorldPosition.Y, size);

                continuousMotion.SetAtLegacyWorldPosition(continuousMotion.WorldPosition, derived);
            ...
            var ingressHex =
                ...
                ingressFootprintHex.HasValue && site.OccupiesLegacyHex(ingressFootprintHex.Value)

                    ? ingressFootprintHex.Value

                    : site.LegacyPresenceHex;
            ...
            // LocalVisibleAutoTravel 均已先 SetLegacyWorldPositionAndHex/SetAtLegacyWorldPosition(boundary)）。

            // 不再用 ResolveLegacyTravelPresentationWorld（presentation 是 travel 中 transient，B3B.1 后

            // 不参与 final context-preserving commit）。禁止 fallback LegacyPresenceHex / LegacyAnchorHex center；

            // 无位置时明确失败并报告链路缺口（不用 magic offset 修）。

            // Phase 5R-B3B.4：统一 ingress trace id —— [1 IngressBoundary]（含 boundary /

            // fromHex=进入前 LegacyCurrentHex / ingressFootprintHex），后续节点（AtSiteCommit /
...

        {
            ...
            // Phase 5B: Mid-Segment / in-flight AutoTravel must not Snap or SetAtLegacyWorldSite.
...

            {
                ...
                // BeginLegacyHexAutoTravel 将把 Site 转为 Presence 中心上的 AtWorldPosition。

                if (world.Strategic.Sites.TryResolveLegacySitePresenceHex(motion.SiteId, out var presence))

                    motion.SetAtLegacyWorldSite(motion.SiteId, presence, hexSize);

                else

                    motion.SetAtLegacyWorldSite(motion.SiteId, startHex, hexSize);
            ...
            if (!motion.HasPosition)
                ...
                motion.SnapToLegacyHexCenter(startHex, hexSize);
...

        {
...

            {
                ...
                if (!HexPathfinder.TryFindPath(
                        ...
                        world.LegacyHexWorld, from, hex, pathScratch, HexTravelMode.Ground, blocked) ||
...

        {
            ...
            var normalSurface = ContinuousOutdoorGameplayPolicy.IsNormalContinuousOutdoor(world) &&
                                ...
                                world.SurfaceGround.TryResolveSiteArrival(siteId, out surfaceId, out _);
...

            {
                ...
                if (normalSurface)
                    ...
                    world.WorldPresence.SetAtSiteWithAnchor(id, siteId,

                        world.PlayerPartyTravel.WorldPosition, surfaceId);
...

        {
            ...
            world.PlayerPartyTravel?.SnapToLegacyHexCenter(hex, hexSize);

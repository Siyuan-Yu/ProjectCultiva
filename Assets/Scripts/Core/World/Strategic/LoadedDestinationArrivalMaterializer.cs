            ...
            PlayerPartyRuntime party)
        {
            ...
            for (var i = 0; i < occupants.Count; i++)
            {
                ...
                if (party != null && party.IsMember(id))
                    continue;
                // Squad-owned Local presentation 是派生缓存
                // （World authority = SquadWorldMotion），LocalMap unload 可以安全 release；
                // 下一张正确地图由 LoadedStrategicPopulationMaterializer 重新 rematerialize。
                // 不清 SquadWorldMotion、不解散 Squad、不修改成员 WorldPresence 世界位置。

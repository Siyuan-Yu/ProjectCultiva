...

{
    ...
    /// 本类公式只解释旧 Hex footprint 输入，不定义 Continuous Surface 或
    ...
    ///    LegacyOccupiedHexes + HexMath Pointy-Top / Odd-R 几何。

    ///  - LegacyAnchorHex / LegacyPresenceHex / DisplayName / SiteType 不参与物理映射。

    ///  - <b>LegacyOccupiedHexes 为空 / kernel 为空时所有 Physical Mapping API 明确失败</b>（不伪造

    ///    LegacyAnchorHex fake single-hex physical domain，不静默 fallback）。

    /// </summary>

    public static class WorldSiteHexFootprintSpatialMapping

    {
        ...
        /// Site footprint 解析：<see cref="WorldSite.LegacyOccupiedHexes"/>（缓存 ReadOnlyCollection，零分配）。

        /// <b>空 footprint 一律返回 false</b> —— LegacyAnchorHex / LegacyPresenceHex 不参与 physical mapping
...

        {
            ...
            footprint = site.LegacyOccupiedHexes;
        ...
        /// 计算 Site footprint 的连续 world-surface domain：全部 LegacyOccupiedHexes 的真实 Hex 角点
        ...
        /// DerivedLegacyPresenceHex：worldPosition → WorldToHex；若 ∈ footprint 直接返回；

        /// 数值边界歧义（polygon boundary / 空洞）用 footprint polygon containment 稳定解析

        /// （返回最近 occupied hex）。不使用 LegacyAnchorHex / LegacyPresenceHex / DisplayName / SiteType。
...

    {
        ...
        /// Phase 5R-B6：仅当 departure 处于 <see cref="LegacyPlayerPartyDeparturePhase.TransitionCommit"/>
...

    {
        ...
        /// <summary><see cref="PlayerPartyWorldMotion.TryUpdateLegacyWorldPositionWithinSite"/> 拒绝
    ...
    /// Local Position（Active Character 当前实际 Local Transform）→ WorldSiteHexFootprintSpatialMapping V2

    /// （复用已构建 geometry，零堆分配）→ PlayerPartyWorldMotion.TryUpdateLegacyWorldPositionWithinSite
    ...
    /// 不修改 LegacyCurrentHex（TryUpdateLegacyWorldPositionWithinSite 语义保证）；LocationKind / SiteId 不变。
...

    {
...

        {
            ...
            if (!motion.TryUpdateLegacyWorldPositionWithinSite(motion.SiteId, canonicalPosition))

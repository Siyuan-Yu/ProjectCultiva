...
namespace XianXia.Tests
{
    ...
    /// 不得再用 AtWorldSite 冻结的 LegacyCurrentHex（= presence）作为 WorldMap 路线前缀起点。
    ...
    public sealed class WorldSiteDepartureRouteConsistencyTests
    {
            ...
            out PlayerPartyWorldMotion motion)
        {
            ...
            motion.SetAtLegacyWorldSite(site.SiteId, site.LegacyPresenceHex, HexSize); // LegacyCurrentHex = presence
            Assert.IsTrue(
                motion.TryUpdateLegacyWorldPositionWithinSite(site.SiteId, canonical),
        ...
        public void B6_3A_01_RouteStart_UsesCanonicalDerivedHex_NotFrozenCurrentHex()
        {
            ...
            // presence=(80,51)；角色 Canonical 在 (81,52)（另一 footprint hex）→ LegacyCurrentHex 冻结为 (80,51)
            ...
            Assert.AreEqual(site.LegacyPresenceHex, motion.LegacyCurrentHex, "fixture LegacyCurrentHex 冻结为 presence");
            ...
            Assert.AreNotEqual(site.LegacyPresenceHex, start, "不再从 presence 画路线前缀");
        ...
        [Test]
        public void B6_3A_02_RouteStart_WhenCanonicalAtLegacyPresenceHex_NoSplit()
        {
            var (world, site, party) = Build();
            ...
            Assert.AreEqual(motion.LegacyCurrentHex, start, "与 LegacyCurrentHex 一致（角色恰在 presence）");
        ...
        public void B6_3A_03_AtWorldPosition_KeepsCurrentHexBehavior()
        {
            ...
            motion.SetAtLegacyWorldPosition(new WorldVec2(wx, wy), new HexCoord(70, 60));
            var path = new List<HexCoord> { new HexCoord(70, 60), new HexCoord(71, 60), new HexCoord(72, 60) };
            motion.BeginLegacyHexAutoTravel(path, new HexCoord(72, 60), string.Empty, HexTravelMode.Ground, HexSize);

            Assert.IsTrue(
                PlayerPartyWorldLocationQuery.TryResolveRouteStartHex(world, motion, out var start, out var pathIndex),
                "resolve route start");
            Assert.AreEqual(new HexCoord(70, 60), start, "AtWorldPosition → LegacyCurrentHex（既有行为）");
            Assert.AreEqual(1, pathIndex, "path[0]==LegacyCurrentHex → 跳过");
        }
        ...
        public void B6_3A_04_NoDeparture_KeepsCurrentHex()
        {
            ...
            // 无 departure：普通 Site 内移动（B4 状态）→ helper 返回 LegacyCurrentHex（保守）
            Assert.IsTrue(
                PlayerPartyWorldLocationQuery.TryResolveRouteStartHex(world, motion, out var start, out var pathIndex),
                "resolve route start");
            Assert.AreEqual(motion.LegacyCurrentHex, start, "无 departure → LegacyCurrentHex");
        ...
        public void B6_3A_05_AuthorityInvariant_Exit_Connection_RouteStart()
        {
            ...
            // LegacySiteDepartureExitHex == FormalConnection.DestinationHex
            // LegacySiteDepartureFootprintHex == FormalConnection.SourceHex
            ...
            Assert.IsTrue(
                WorldSiteFootprintExitConnectionResolver.TryResolveFormalExitConnection(
                    world, site, motion.LegacySiteDepartureFootprintHex, motion.LegacySiteDepartureExitHex, HexSize,
                    RealBounds, out var conn),
                "formal connection resolve");

            Assert.AreEqual(motion.LegacySiteDepartureExitHex, conn.DestinationHex,
                "Route exit hex == FormalConnection.DestinationHex");
            Assert.AreEqual(motion.LegacySiteDepartureFootprintHex, conn.SourceHex,
                "Route footprint hex == FormalConnection.SourceHex");
            Assert.AreEqual(derived, motion.LegacyHexPath[0], "path[0] == Canonical derived hex");
            Assert.IsTrue(site.OccupiesLegacyHex(derived), "canonical 派生 hex 在 footprint 内");
        }
    }
}

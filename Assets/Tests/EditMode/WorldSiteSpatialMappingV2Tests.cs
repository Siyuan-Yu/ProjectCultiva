...

{
    ...
    ///  - LegacyAnchorHex 改变不影响 mapping；LegacyPresenceHex 不参与

    /// </summary>

    public sealed class WorldSiteHexFootprintSpatialMappingV2Tests

    {
        ...
        static (WorldSite Site, WorldSiteHexFootprintSpatialMapping.WorldSiteLocalMapBounds Bounds, bool[] Blocked, int Width, int Height) LoadReferenceWorld()

        {
            ...
            var bounds = new WorldSiteHexFootprintSpatialMapping.WorldSiteLocalMapBounds(originX, originY, cellSize, width, height);
...

        {
...

            {
                ...
                var site = new WorldSite { SiteId = siteId, LegacyAnchorHex = hexes[0] };

                site.SetLegacyHexFootprint(hexes);

                Assert.IsTrue(

                    WorldSiteHexFootprintSpatialMapping.TryBuildGeometry(site, HexSize, out var g) && g.HasKernel,

                    siteId + " kernel empty (disconnected or non-star-shaped footprint)");

                Assert.IsTrue(WorldSiteHexFootprintValidator.IsFootprintConnected(hexes), siteId + " disconnected");
...

        {
            ...
            Assert.IsTrue(WorldSiteHexFootprintSpatialMapping.TryBuildGeometry(site, HexSize, out var g));
...

            {
...

                {
                    ...
                    Assert.IsTrue(
                        ...
                        WorldSiteHexFootprintSpatialMapping.TryLocalToWorldSurface(g, bounds, p, out var w),

                        "L2W fail at (" + cx + "," + cy + ")");

                    Assert.IsTrue(

                        WorldSiteHexFootprintSpatialMapping.TryWorldSurfaceToLocal(g, bounds, w, out var p2),
...

        {
...

            {
                ...
                Assert.IsTrue(
                    ...
                    WorldSiteHexFootprintSpatialMapping.TryWorldSurfaceToLocal(g, bounds, contact, out var loc),
...

        {
...

            {
                ...
                Assert.IsTrue(WorldSiteHexFootprintSpatialMapping.TryBuildGeometry(site, HexSize, out var g), sid + " geometry");

                Assert.IsTrue(g.HasKernel, sid + " kernel");

                // 真实 map bounds（ch01_site_a_map.json 与 reference 同尺寸）

                var bounds = new WorldSiteHexFootprintSpatialMapping.WorldSiteLocalMapBounds(-40f, -25f, 1f, 200, 100);
...

                {
...

                    {
                        ...
                        if (!WorldSiteHexFootprintSpatialMapping.TryLocalToWorldSurface(g, bounds, p, out var w))
                        ...
                        if (!WorldSiteHexFootprintSpatialMapping.TryWorldSurfaceToLocal(g, bounds, w, out var p2))
...

        {
            ...
            disconnected.SetLegacyHexFootprint(new[] { new HexCoord(0, 0), new HexCoord(2, 0) });

            var errors = WorldSiteHexFootprintValidator.ValidateFootprint(disconnected, HexSize);
            ...
            concave.SetLegacyHexFootprint(new[]
            ...
            var errs2 = WorldSiteHexFootprintValidator.ValidateFootprint(concave, HexSize);
            ...
            var errs3 = WorldSiteHexFootprintValidator.ValidateFootprint(empty, HexSize);
...

        {
            ...
            var a1 = new WorldSite { SiteId = "a1", LegacyAnchorHex = new HexCoord(80, 52) };

            a1.SetLegacyHexFootprint(baseFp);

            var a2 = new WorldSite { SiteId = "a2", LegacyAnchorHex = new HexCoord(81, 51) };

            a2.SetLegacyHexFootprint(baseFp);

            Assert.IsTrue(WorldSiteHexFootprintSpatialMapping.TryBuildGeometry(a1, HexSize, out var g1));

            Assert.IsTrue(WorldSiteHexFootprintSpatialMapping.TryBuildGeometry(a2, HexSize, out var g2));
...

            {
                ...
                WorldSiteHexFootprintSpatialMapping.TryLocalToWorldSurface(g1, bounds, samples[i], out var w1);

                WorldSiteHexFootprintSpatialMapping.TryLocalToWorldSurface(g2, bounds, samples[i], out var w2);
...

        {
...

            {
                ...
                WorldSiteHexFootprintSpatialMapping.TryLocalToWorldSurface(g, bounds, warm, out var ww);

                WorldSiteHexFootprintSpatialMapping.TryWorldSurfaceToLocal(g, bounds, ww, out _);

...
{
    /// <summary>HEX-ATTACK / phantom occupancy：Pure Hex 敌军占格仅认 FormalArmy.CurrentHex。</summary>
    public sealed class HexActiveEnemyArmyQueryTests
    {
        ...
        static FormalArmy SeedBanditWithLegacyNodeAtEnemyHex(SimulationWorld world)
        {
            ...
            Assert.AreEqual(NodeHuangcun, stack.NodeId, "legacy stack.NodeId must stay polluted for regression");
        ...
        [Test]
        public void HEX_PHANTOM_01_LegacyNodeId_DoesNotOccupySiteHex()
        {
            var world = CreateWorld();
            SeedBanditWithLegacyNodeAtEnemyHex(world);

            var atSite = Collect(world, SiteHex);
            Assert.AreEqual(0, atSite.Count, "phantom enemy at site hex via NodeId fallback");
            ...
            Assert.AreEqual("荒村山匪", atEnemy[0].DisplayName);
        }

        [Test]
        public void HEX_MOVE_01_RelocateEnemy_UpdatesOccupancyWithoutTouchingNodeId()
        {
            var world = CreateWorld();
            var enemy = SeedBanditWithLegacyNodeAtEnemyHex(world);

...

{
    ...
    /// DerivedHex = WorldToHex(Canonical)；non-finite/no-position → LegacyPresenceHex legacy fallback（只读）。
...

    {
...

        {
            ...
            world.LegacyHexWorld.FillRectangle(140, 100, HexTerrainType.Plain);
...

        {
            ...
            m.SetAtLegacyWorldSite(site.SiteId, site.LegacyPresenceHex, HexSize);
...

        {
            ...
            Assert.AreEqual(hex, world.PlayerPartyTravel.LegacyCurrentHex, "LegacyCurrentHex 未变");
        ...
        // ============================ [2] Query 不再返回 LegacyPresenceHex center ============================



        [Test]

        public void B5_02_ValidCanonical_NotLegacyPresenceHexCenter()

        {
            ...
            HexMath.ToWorldPosition(site.LegacyPresenceHex, HexSize, out var px, out var py);
            ...
            Assert.AreNotEqual(presenceCenter, r.WorldPosition, "不再返回 LegacyPresenceHex center");
...

        {
            ...
            // 右缘 canonical：derivedHex 应明显非 LegacyPresenceHex（覆盖 footprint 右半区）
            ...
            Assert.AreNotEqual(site.LegacyPresenceHex, r.DerivedHex,

                "valid Canonical 时 DerivedHex 不再 = LegacyPresenceHex（右缘 canonical 明显不同 hex）");
...

        {
            ...
            AssertUnchangedAfterQuery(world, m.LocationKind, m.SiteId, m.WorldPosition, m.LegacyCurrentHex);
        ...
        // ============================ [9] LegacyPresenceHex 改变 → valid Canonical query 不变 ============================



        [Test]

        public void B5_09_LegacyPresenceHexChange_DoesNotAffectValidCanonicalQuery()

        {
            ...
            site.LegacyPresenceHex = newPresence;

            Assert.IsTrue(PlayerPartyWorldLocationQuery.TryResolve(world, null, out var after), "resolve after");



            Assert.AreEqual(before.WorldPosition, after.WorldPosition, "LegacyPresenceHex 改变不影响 valid Canonical query");
        ...
        // ============================ [10] LegacyAnchorHex 改变 → valid Canonical query 不变 ============================
...

        {
            ...
            site.LegacyAnchorHex = new HexCoord(81, 51); // footprint 内另一 hex

            Assert.IsTrue(PlayerPartyWorldLocationQuery.TryResolve(world, null, out var after), "resolve after");



            Assert.AreEqual(before.WorldPosition, after.WorldPosition, "LegacyAnchorHex 改变不影响 valid Canonical query");
...

        {
            ...
            Assert.IsTrue(
                ...
                m.TryUpdateLegacyWorldPositionWithinSite(site.SiteId, new WorldVec2(float.NaN, 76f)),
            ...
            Assert.AreEqual(new WorldVec2(px, py), r.WorldPosition, "legacy fallback = LegacyPresenceHex center（只读输出）");

            Assert.IsTrue(r.IsLegacyFallback, "fallback 标记");

            Assert.AreEqual(site.LegacyPresenceHex, r.DerivedHex, "fallback DerivedHex = LegacyPresenceHex");
...

        {
            ...
            m.SetAtLegacyWorldPosition(new WorldVec2(50f, 50f), hex);
...

        {
            ...
            m.BeginLegacyHexAutoTravel(
                ...
                new List<HexCoord> { site.LegacyPresenceHex, new HexCoord(79, 51) },
            ...
            Assert.AreEqual(m.ResolveLegacyTravelPresentationWorld(HexSize), r.WorldPosition, "travel presentation 位置");

            Assert.AreEqual(m.LegacyCurrentHex, r.DerivedHex, "moving 时 DerivedHex = LegacyCurrentHex");
...

    {
        ...
        static WorldSiteHexFootprintSpatialMapping.WorldSiteLocalMapBounds LoadReferenceBounds()

        {
            ...
            return WorldSiteHexFootprintSpatialMapping.WorldSiteLocalMapBounds.FromOriginSize(
        ...
        static HexFootprintSpatialGeometry BuildGeometry(out WorldSiteHexFootprintSpatialMapping.WorldSiteLocalMapBounds bounds)

        {
            ...
            Assert.IsTrue(WorldSiteHexFootprintSpatialMapping.TryBuildGeometry(site, HexSize, out var g), "huangcun geometry");
...

        {
            ...
            motion.SetAtLegacyWorldSite("base:site_huangcun", new HexCoord(80, 51), HexSize);
...

        {
            ...
            var before = motion.LegacyCurrentHex;
            ...
            Assert.AreEqual(before, motion.LegacyCurrentHex, "B4 禁止写 LegacyCurrentHex（留 5R-C）");
...

        {
            ...
            // TryUpdateLegacyWorldPositionWithinSite 自身防御：expected != motion.SiteId → 拒绝。

            var rejected = motion.TryUpdateLegacyWorldPositionWithinSite("base:site_daoguan", new WorldVec2(200f, 200f));
            ...
            wildernessMotion.SetAtLegacyWorldPosition(new WorldVec2(10f, 10f), new HexCoord(1, 1));
...

        {
...

            {
                ...
                Assert.IsTrue(
                    ...
                    WorldSiteHexFootprintSpatialMapping.TryWorldSurfaceToLocal(g, bounds, canonical, out var back),
...

        {
            ...
            idle.SetAtLegacyWorldSite("base:site_huangcun", new HexCoord(80, 51), HexSize);



            var moving = new PlayerPartyWorldMotion();

            moving.SetAtLegacyWorldSite("base:site_huangcun", new HexCoord(80, 51), HexSize);

            moving.BeginLegacyHexAutoTravel(
...

        {
...

            {
                ...
                var outcome = PlayerPartyWorldSiteLocalVisibleSync.TrySync(motion, g, new WorldSiteHexFootprintSpatialMapping.WorldSiteLocalMapBounds(

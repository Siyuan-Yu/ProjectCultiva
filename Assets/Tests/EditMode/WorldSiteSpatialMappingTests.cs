...

{
    ...
    /// Phase 5R-B1：WorldSiteHexFootprintSpatialMapping 纯 Core 数学测试（EditMode / 无运行时接线）。

    /// 覆盖：单 Hex 中心映射与 roundtrip、规则多 Hex 方向性、不同 LocalMap 尺寸的

    /// normalized 语义、irregular footprint 投影、DerivedLegacyPresenceHex 恒 ∈ footprint。

    /// </summary>

    public sealed class WorldSiteHexFootprintSpatialMappingTests

    {
...

        {
...

            {
                ...
                LegacyAnchorHex = footprint.Length > 0 ? footprint[0] : default(HexCoord),
        ...
        static WorldSiteHexFootprintSpatialMapping.WorldSiteLocalMapBounds Bounds(

            float cellSize, int width, int height, float originX = 0f, float originY = 0f) =>

            WorldSiteHexFootprintSpatialMapping.WorldSiteLocalMapBounds.FromOriginSize(originX, originY, cellSize, width, height);



        static WorldSiteHexFootprintSpatialMapping.WorldSiteLocalMapBounds Bounds50x50 => Bounds(1f, 50, 50);

        static WorldSiteHexFootprintSpatialMapping.WorldSiteLocalMapBounds Bounds100x80 => Bounds(1f, 100, 80);
...

        {
            ...
            var ok = WorldSiteHexFootprintSpatialMapping.TryLocalToWorldSurface(
            ...
            Assert.IsTrue(site.OccupiesLegacyHex(HexMath.WorldToHex(world.X, world.Y, HexSize)),
...

        {
...

            {
                ...
                Assert.IsTrue(WorldSiteHexFootprintSpatialMapping.TryLocalToWorldSurface(

                    site, Bounds50x50, local, HexSize, out var world), "L2W " + local);

                Assert.IsTrue(WorldSiteHexFootprintSpatialMapping.TryWorldSurfaceToLocal(
...

        {
            ...
            Assert.Less(0f, WorldSiteHexFootprintSpatialMapping.TryComputeFootprintWorldDomain(
...

        {
            ...
            Assert.IsTrue(WorldSiteHexFootprintSpatialMapping.TryResolveDerivedFootprintHex(
        ...
        // ---- 5. DerivedLegacyPresenceHex 恒 ∈ footprint（多采样点） ----
...

        {
            ...
            var footprint = new HashSet<HexCoord>(site.LegacyOccupiedHexes);
...

            {
                ...
                Assert.IsTrue(footprint.Contains(derived),
                    ...
                    "DerivedLegacyPresenceHex " + derived + " must be inside footprint for local " + local);
...

        {
            ...
            Assert.IsFalse(WorldSiteHexFootprintSpatialMapping.TryLocalToWorldSurface(

                null, Bounds50x50, new WorldVec2(25f, 25f), HexSize, out _));

            Assert.IsFalse(WorldSiteHexFootprintSpatialMapping.TryLocalToWorldSurface(

                site, default(WorldSiteHexFootprintSpatialMapping.WorldSiteLocalMapBounds),
            ...
            Assert.IsFalse(WorldSiteHexFootprintSpatialMapping.TryResolveDerivedFootprintHex(
...

        {
            ...
            motion.SetAtLegacyWorldSite("test:site_guanai", new HexCoord(12, 11), HexSize);

            var target = new WorldVec2(3.5f, -2.25f);



            Assert.IsTrue(motion.TryUpdateLegacyWorldPositionWithinSite("test:site_guanai", target),
            ...
            Assert.AreEqual(new HexCoord(12, 11), motion.LegacyCurrentHex,

                "LegacyCurrentHex 不得被本 API 改动（职责分类留 5R-C）");
...

        {
            ...
            Assert.IsFalse(motion.TryUpdateLegacyWorldPositionWithinSite("test:site_lingdi", new WorldVec2(99f, 99f)),
...

        {
            ...
            motion.SetAtLegacyWorldPosition(new WorldVec2(5f, 5f), new HexCoord(0, 0));



            Assert.IsFalse(motion.TryUpdateLegacyWorldPositionWithinSite("test:site_guanai", new WorldVec2(99f, 99f)),
...

        {
            ...
            // LegacyAnchorHex 有效但无合法 footprint（LegacyOccupiedHexes 为空；不调用 SetLegacyHexFootprint）。

            // ADR-0027：LegacyAnchorHex 不是 Physical Position / Spatial Mapping authority，
...

            {
                ...
                LegacyAnchorHex = new HexCoord(3, 4),
            ...
            Assert.IsTrue(site.LegacyAnchorHex.Equals(new HexCoord(3, 4)), "precondition: anchor must be valid");

            Assert.AreEqual(0, site.LegacyOccupiedHexes.Count, "precondition: no legal footprint");
            ...
            Assert.IsFalse(
                ...
                WorldSiteHexFootprintSpatialMapping.TryResolveDerivedFootprintHex(
...

        {
            ...
            motion.SetAtLegacyWorldPosition(continuous, new HexCoord(11, 11));



            Assert.IsTrue(

                motion.TrySetAtLegacyWorldSitePreservingWorldPosition("test:site_guanai", continuous),
            ...
            Assert.AreEqual(new HexCoord(11, 11), motion.LegacyCurrentHex,

                "LegacyCurrentHex 不得被本 API 改动（三职责分类留 5R-C）");
...

        {
            ...
            Assert.IsFalse(
                ...
                motion.TrySetAtLegacyWorldSitePreservingWorldPosition("", new WorldVec2(1f, 1f)),
            ...
            Assert.IsFalse(
                ...
                motion.TrySetAtLegacyWorldSitePreservingWorldPosition(
...

        {
...

            {
                ...
                LegacyPresenceHex = fp,

            };

            site.SetLegacyHexFootprint(new[] { fp });
...

            {
                ...
                world.LegacyHexWorld.SetTile(new HexCell { Coord = ext });

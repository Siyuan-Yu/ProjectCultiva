...
**已于 2026-07-31 进入原型开发阶段。** 当前只实现 `docs/40-process/45-demo-v0.1.md` 冻结的垂直切片，并采用可替换 Sprite 与占位素材，不等待最终美术。

- 允许建立 Unity 工程、原型代码、场景、Prefab 与占位资源。
- 原型实现必须服务 Demo 闭环，不得借机扩展完整游戏。
- 未定设计仍标记为「待确定」，不得为了写代码自行拍板。
- 角色、建筑、地块与 UI 图标必须通过独立 Sprite 引用，保证 PNG 可原位替换。
- 不要新增未经讨论的核心机制；也不要因为一次更新就删掉之前已确认的设计。

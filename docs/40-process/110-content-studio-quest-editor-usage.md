...
> 状态：**可用（WPF v2 · 可视化条件）**｜日期：2026-08-13  
...
新建／改任务：**基本信息、发放方式、接取／完成／失败条件、奖励**。  
条件与奖励用 **可视化列表**（`+ 添加`）编辑，不必手写 JSON。  
NPC 对话发任务：在本编辑器创建关联事件，**台词在 EventEditor 改**。
...
`completeConditions`：**全部满足**则任务完成。类型与 ③ 相同（常见如 **已探索地点**、**任务已完成**、**已有 Flag** 等）。

**示例（ch01 巡视杂役区）：**

- 条件：**已探索地点** → `base:loc_ref_labor_yard`  
  表示玩家探索过杂役区即算完成巡视。

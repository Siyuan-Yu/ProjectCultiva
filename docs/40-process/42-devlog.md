...
- 所有 Continuous outdoor source chunk 改用一个精确矩形背景 renderer，草地背景从每块625个降为1个；交互 placements、墙/门/物件和导航不走背景合并。后续已确认 500×500 指 Surface cells，非 world 单位。
...
- 正式加入预检位置和资格，再追加实际名单与保留原 HP 基线的报告投影；版本随合法加入递增。候选不提前显示、受伤或计胜负；最多一个有界待到达续战窗口，结算关闭余下候选。
- 候选状态、原位置、roll、到达期限、规则快照和名单版本进入正式 JSON；现有开发工具战斗页可观察并做受关系/范围资格约束的决定。
- C2 检查点 a8b322a；本阶段离线编译与静态检查通过后继续 C4，未启动 Unity 或测试。

## 2026-09-13 — C2/U2B：人物统一攻击与玩家远程攻击退役

- 普通人物和 Army 成员统一 LocalCharacter，初始只取双方当前小队；到达攻击与旧近战适配转交 HostCharacterEncounter。最终伤害门禁在玩家尚未入场时挂起请求，阻止第一刀绕过确认；多人各自持有目标和冷却。
- 玩家 WorldMap 攻击菜单、执行函数、PlayerParty 命令资格退役；玩家 Army 攻击/旧档恢复/旅行后追击回调取消旧攻击 intent。普通旅行与 AI 方 Army 战略命令保留。
- C1 检查点 ccdadeb。本阶段 Core459/Data78/Host145 离线编译通过，未运行 Unity 或自动测试；继续 C3。

## 2026-09-13 — C1/U2A：统一核心范围与独立人物遭遇首条默认入口

- 当时把制作人所述500×500错记为world单位；2026-09-14最终锁定Site Level 1为150×150 Surface cells（Main Surface为4.2×4.2 world／3×3 chunks），Wilderness Encounter独立保留500×500 cells。
- 普通人物右键默认进入 CharacterEncounter 确认与独立场地实例，沿用真实 Character 伤害和战报；范围从真实核心或野外配置冻结，个人原锚点与战术状态进入正式 JSON，稳定战中/可结束恢复时重建场地。
- 本场时间只消费参战者生命期限；主世界不推进。返回只恢复空间，保留伤亡与消耗，报告单独持有暂停。
- Core459/Data78/Host145 现成离线编译通过；继续 C2→C4，不逐阶段等待人工确认。实现细节及后续状态见 222；未运行 Unity、测试或 Bake。
...
- C0 修复离队／解散的前置验证与成员事务、创建边界 singleton、严格 Squad 新档及控制预绑定、共同命令／日程 ownership、保留身份但停止非存活成员移动。未改地图内容、暂停 ownership 或战斗结算。
- 现成离线编译 Core 456／Data 77／Unity Host 143 源文件通过，保留既有 warning；未启动 Unity、自动测试或 Bake。C1～C4 尚不能按此条认定完成。

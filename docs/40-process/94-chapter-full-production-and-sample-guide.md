...
| W 或底栏「劳动」后左键工区 | 走到区中心 → **抵达后才劳动** |
| 底栏「入定」后左键灵地 | 走到区中心 → **抵达后才入定** |
| 已在工区时点「劳动」／F4 | 当场劳动（无需再走） |
| 右键灵泉／洞府 | 走到区中心 → **抵达后入定** |
| 右键空地 | 只移动 |
| 底栏劳役／入定（或 W） | 进入点选：再**左键**点工区／灵地（同戴帽版） |
| 底栏宣纸面板 | **点选一名己方角色才打开**；按钮／F 键**只令该角色**（非全队） |
| F1–F4／F6–F8 | 休息／观察／探索／劳动／入定／出定／帮助（F5 留给存档） |
| 左附属格 | 停止／敛息／劳动（同上，仅焦点角色） |
| 右页签 况／系 | 该角色概况／关系（课表／任务／事件在**右侧全局栏**） |
| 顶栏 | 暂停／倍速／库存／主管愤怒（全局） |
| 框选多人 | 底栏仍只控焦点那一人；群体位移用右键 |
| C／X／G／S | 入定／出定／敛息／停止 |
| F10 | 显隐 FormalHud |
| F5／F9 | 存／读（按钮默认隐藏） |
...
1. 开局弹层「杂役晨点」（含操作说明）→ 任务「知道了」→ 已预选第一人：F3／底栏「探索」巡视农田（或先走开再走回，首次入区会自动勘察）  
2. 走到「树林」后点「劳动」／F4（或 W 再点树林）交差 → 药田同理  
3. 走进「灵泉」听异响 → 再进树林遇砍柴老人（替代：枢纽再按一次探索找行商）  
4. 进洞府后点「入定」学青云诀 → 再进洞府选「引气入体」→ 多次入定至炼气  
5. 进房屋区立约隐藏 → 回枢纽听制度伏笔  

常驻提示：顶栏下方操作条；调试面板改为 **F11**（不再与 F1 休息冲突）。  

...
普通 Development 实施完成门槛以 `AGENTS.md` 为准：没有基础编译错误，加上最少量静态引用／文件完整性检查即可。**不自行运行测试，不自行启动 Unity，不以测试通过作为实施完成条件。** 编译、静态核对与制作人人工验收分开记录。普通实施完成后不自动提交；制作人明确要求“封板”时即授权选择性 `git add` + `git commit`，默认不 push、不打 tag。

### 4.1 单轮执行与状态纪律

- 一轮只保留一份明确可复制的实施指令；直接接线随同一轮完成，不按文件拆成重复验收。
- 有运行改动时提供简短、可操作且与范围对应的人工验收；纯文档修改不要求进入 Unity。
- 执行代理不编写或运行自动测试，不启动 Unity。普通代码实施只做基础编译与最少量静态引用／文件完整性检查。
- **状态必须分层：** `Design Confirmed` 只表示规则已确认；`Implemented` 只表示代码已接通；`Producer Accepted` 必须有明确人工验收；`Committed / Sealed` 必须有真实 Git 记录与封板授权；`Proposed / Not Implemented` 不构成实施许可。
- 编译、静态核对、历史测试记录和制作人人工验收分别记录，互不替代。历史测试事实可保留，但不得据此要求后续代理重跑。
- 普通实施不自动提交；“封板”即授权选择性提交。默认不 push／tag／amend／reset／clean，不混入无关工作区改动。

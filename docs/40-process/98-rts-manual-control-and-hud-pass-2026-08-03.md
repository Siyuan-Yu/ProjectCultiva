...
- 顶栏时钟：`HH:mm`（每 Tick＝**5** 游戏分钟；1 日＝288 Tick；1x 每跳 5 分）
...
2. 己方空闲不自行变巡查／工作  
3. 点交互 → **仅**黄点绿光标，麦田其它位置红；左键黄点→走到后工作  
4. 选中己方不点交互：悬停黄/青点变可交互光标，**右键**＝前往并劳动／修炼；空地右键＝只移动  
5. 点守卫：仅查看，无上方指令  

## 8. 仍缺的基础 RTS 交互（审计）

已补：武装绿红、情境悬停＋右键交互、多交互点、框选／点选己方、右键移动、停止。

尚未做或很弱：

| 项 | 说明 |
|----|------|
| 攻击移动／战斗右键 | 战斗仅占位，无右键攻击 |
| 队列／Shift 排队指令 | 新令直接打断，无路点队列 |
| 巡逻／守卫指令 | 无玩家下达的巡逻路线 |
| 建筑／生产队列 UI | 无 |
| 小地图点击移动 | 无小地图 |
| 控制组 Ctrl+1.. | 无 |
...
| 路径预览线 | 移动无画线 |
| 不可达／红格反馈 | 全图可点移动，无障碍 |
| 多选分别交互 | 框选后指令仍主要落焦点一人 |
| NPC 情境菜单 | 右键 NPC 只走近，无对话树 |

...
> 状态：脚本已就绪 | 最后更新：2026-08-01  
> 文件编号：先后为 `35`→`36`→现 **`37-feishu-sync.md`**（`35`=Order／Action，`36`=ContentPackage／Mod Ready）。
...
**真源：** 仓库内 Markdown。**飞书：** 阅读／分享层，由 `tools/feishu-map.json` 映射。

同步范围（设计正文）：

- `docs/00-project/` 全部（含通读指南）
- `docs/10-benchmark/` 全部
- `docs/20-systems/` 全部系统页＋索引
- `docs/30-tech/` 架构与同步说明（含 `33` v0.2／`34`／`35`／`36`）
- `docs/40-process/` 路线图、日志、Demo、审计、交接、**全部 ADR＋索引**

**不同步（刻意）：** `docs/90-templates/`（写作模板）、过期交接副本（如 `44-session-handoff-2026-07-30.md`）、仓库根 `AGENTS.md` 等工程提示。

新增文档流程：改 map → `--provision` →（如需）`--share --openid` → 全量同步。

飞书文档**标题**由飞书侧／建档时标题维护；正文以本地为准。若飞书标题仍显示旧版本号（例如仍写 v0.1），可在飞书里手动改标题，不影响正文同步。

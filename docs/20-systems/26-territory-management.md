...
WorldMap 正常产品势力范围直接绘制 Actual Administrative Control 的 world-space 矩形片段及真实边界，不从 `HexCell.ControlFactionId`／`TerritoryRegion.Hexes` 反推轮廓。CW-04.5 已实现并封板按 Faction 的只读视觉 union：同势力相邻 Site 不画内部政治边界，断开的领土仍是独立 pieces；Site identity、Claim 与唯一 Actual Manager 保持独立，内部 Site 边界继续由 per-Site builder 和 diagnostics 查询。正常产品已无 Strategic Hex Projection runtime authority；历史 Hex 只存在于明确旧输入／wire／离线转换或历史资料边界。

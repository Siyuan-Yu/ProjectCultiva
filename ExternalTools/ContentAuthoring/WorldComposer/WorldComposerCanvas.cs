...
public sealed class WorldComposerCanvas : SurfaceCanvasBase
{
    ...
    private PreviewRect? _dirtySurface,_pendingDirtyOverlay;
    ...
    public void RemoveSelectedControlPoint(){if(_selectedPoint is not{}p)return;BeginMutation(true);foreach(var path in Document.Rivers.Cast<object>().Concat(Document.Roads)){var points=path is RiverPathSource r?r.ControlPoints:((RoadPathSource)path).ControlPoints;if(points.Count>2&&points.Remove(p)){_mutationChanged=true;break;}}_selectedPoint=null;EndMutation();}
    public void InsertControlPoint(){if(SelectedItem is not RiverPathSource&&SelectedItem is not RoadPathSource)return;var points=SelectedItem is RiverPathSource r?r.ControlPoints:((RoadPathSource)SelectedItem).ControlPoints;if(points.Count<2)return;var selected=Math.Max(0,_selectedPoint==null?points.Count-2:points.IndexOf(_selectedPoint));var next=Math.Min(points.Count-1,selected+1);BeginMutation(true);var p=new PathControlPoint{X=(points[selected].X+points[next].X)/2,Y=(points[selected].Y+points[next].Y)/2};points.Insert(next,p);_selectedPoint=p;_mutationChanged=true;EndMutation();}
    ...
    protected override void DrawSurface(DrawingContext dc)
    {
        ...
        foreach(var p in Document.WorldSiteBlueprintPlacements){var missing=!Linked.Blueprints.TryGetValue(p.PlacementId,out var b);var w=b?.WidthCells??20;var h=b?.HeightCells??20;if(p.RotationQuarterTurns%2!=0)(w,h)=(h,w);dc.DrawRectangle(missing?Brushes.IndianRed:BlueprintBrush,BlueprintPen,WorldRect(p.SurfaceCellX,p.SurfaceCellY,w,h));DrawLabel(dc,p.BlueprintId,p.SurfaceCellX,p.SurfaceCellY+h);if(b!=null)foreach(var o in b.ObjectPlacements){var q=RotateLocal(o.LocalSurfaceX,o.LocalSurfaceY,b.WidthCells,b.HeightCells,p.RotationQuarterTurns);dc.DrawRectangle(ObjectBrush,ObjectPen,WorldRect(p.SurfaceCellX+q.X,p.SurfaceCellY+q.Y,Math.Max(1,o.WidthCells),Math.Max(1,o.HeightCells)));}}
        ...
        if(_pendingDirtyOverlay is{}dirty)dc.DrawRectangle(HoverBrush,null,WorldRect(dirty.X,dirty.Y,dirty.Width,dirty.Height));
    ...
    private void MarkDirtyEditorCell(int x,int y){var r=ClipRegion(new((x-1)*10,(y-1)*10,30,30));_dirtySurface=_dirtySurface.HasValue?Union(_dirtySurface.Value,r):r;_pendingDirtyOverlay=_dirtySurface;}
    ...
    private async void RequestPreview(PreviewRect? requested)
    {
        ...
        try
        {
            ...
            _pendingDirtyOverlay=null;PreviewStatusChanged?.Invoke("世界预览已更新");InvalidateVisual();
        }
    ...
    private static (int X,int Y) RotateLocal(int x,int y,int w,int h,int turns)=>(((turns%4)+4)%4) switch{0=>(x,y),1=>(h-1-y,x),2=>(w-1-x,h-1-y),_=>(y,w-1-x)};
